...
import { deleteDayFromDatabase } from "@/app/_lib/actions/dataActions";
import NoteButton from "@/app/_ui/NoteButton";
import { FormEvent } from "react";
import { useAppSelector } from "@/app/hooks";
// import { deleteDayFromDatabase } from "@/app/_lib/service/actions copy";
...
function SavedDayHeader({ day }: { day: DayObjectType }) {
    ...
    function handleDelete(e: FormEvent<HTMLFormElement>) {
        ...
        if (window.confirm("Izbriši dan?")) {
            try {
                deleteDayFromDatabase(daysCollection, day._id);
                toast.success("Dan je bil uspesno zbrisan!");
            } catch (err) {
                console.log(err);
            }
            ...
            <div className="flex flex-row gap-3 p-1">
                ...
                <div>
                    <p
                        className="ml-4"
                        style={{
                            color: day.betterPoints >= 0 ? "green" : "red",
                        }}
                    >
                        {day.betterPoints &&
                            `${day?.betterPoints?.toFixed(1)}%`}
                    </p>
                </div>
